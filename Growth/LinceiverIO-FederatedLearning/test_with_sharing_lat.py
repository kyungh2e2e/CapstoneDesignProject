...
class SharedLatentPerceiver(nn.Module):
    def __init__(self,
                 num_latents=64,
                 latent_dim=512,
                 depth=2,
    ...
    def forward(self, bsz):
        x = repeat(self.latents, 'n d -> b n d', b=bsz)
...
class PerceiverHead(nn.Module):
    def __init__(self,
        ...
        self.cross_attn = LinformerAttention(
            dim=latent_dim,
            seq_len=seq_len,
            k=k,
            heads=cross_heads,
        ...
        self.decoder_cross_attn = LinformerAttention(
            dim=queries_dim,
            seq_len=latent_dim,
            k=k,
            heads=cross_heads,
    ...
    def forward(self, data_tokens, queries, shared_latents):
        x = self.cross_attn(shared_latents, context=data_tokens) + shared_latents
        x = self.cross_ff(x) + x
